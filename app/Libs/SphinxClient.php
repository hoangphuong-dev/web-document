...
class SphinxClient
{
    ...
    public function __construct(
        public string $host = 'localhost',          // searchd host (default is "localhost")
        public int $port = 9312,          // searchd port (default is 9312)
        public bool $path = false,          // socket path
        public bool $socket = false,        // socket connection
        public int $offset = 0,        // how many records to seek from result-set start (default is 0)
        public int $limit = 20,         // how many records to return from result-set starting at offset (default is 20)
        public int $mode = self::SPH_MATCH_ALL,          // query matching mode (default is SPH_MATCH_ALL)
        public array $weights = [],       // per-field weights (default is 1 for all fields)
        public int $sort = self::SPH_SORT_RELEVANCE,          // match sorting mode (default is SPH_SORT_RELEVANCE)
        public string $sortby = '',        // attribute to sort by (default is "")
        public int $minid = 0,         // min ID to match (default is 0, which means no limit)
        public int $maxid = 0,         // max ID to match (default is 0, which means no limit)
        public array $filters = [],       // search filters
        public string $groupby = '',       // group-by attribute name
        public int $groupfunc = self::SPH_GROUPBY_DAY,     // group-by function (to pre-process group-by attribute value with)
        public string $groupsort = '@group desc',     // group-by sorting clause (to sort groups in result set with)
        public string $groupdistinct = '', // group-by count-distinct attribute
        public int $maxmatches = 1000,    // max matches to retrieve
        public int $cutoff = 0,        // cutoff to stop searching at (default is 0)
        public int $retrycount = 0,    // distributed retries count
        public int $retrydelay = 0,    // distributed retries delay
        public array $anchor = [],        // geographical anchor point
        public array $indexweights = [],  // per-index weights
        public int $ranker = self::SPH_RANK_PROXIMITY_BM25,        // ranking mode (default is SPH_RANK_PROXIMITY_BM25)
        public string $rankexpr = '',      // ranking mode expression (for SPH_RANK_EXPR)
        public int $maxquerytime = 0,  // max query time, milliseconds (default is 0, do not limit)
        public array $fieldweights = [],  // per-field-name weights
        public array $overrides = [],     // per-query attribute values overrides
        public string $select = '*',        // select-list (attributes or expressions, with optional aliases)
        public string $error = '',         // last error message
        public string $warning = '',       // last warning message
        public bool $connerror = false,     // connection error vs remote error flag
        public array $reqs = [],          // requests array for multi-query
        public string $mbenc = '',         // stored mbstring encoding
        public bool $arrayresult = false,   // whether $result["matches"] should be a hash or an array
        public int $timeout = 0       // connect timeout
    ) {
    }
     ...
     */
    public static function create(): SphinxClient
    {
        return new static();
    }
     ...
     * @param  int  $attrtype attribute type
